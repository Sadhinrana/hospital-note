                            ...
                            <tr>
                                <td>
                                    @if(auth()->user()->role_id == 1 || auth()->user()->role_id == 2 || auth()->user()->role_id == 6)
                                        <a href="{{route('companydetails.show', $company->id)}}">{{$company->name}}</a>
                                    @else
                                        {{$company->name}}
                                    @endif
                                </td>
                                ...
                                @if(auth()->user()->role_id == 1)
                                    @php
                                        $patients = \App\User::where('role_id', 5)->whereIn('user_id', $company->users->pluck('id'))->pluck('id');
                                    @endphp
                                <td>{{ \App\Sms::whereIn('user_id', $patients)->count() }}</td>
                                @endif
