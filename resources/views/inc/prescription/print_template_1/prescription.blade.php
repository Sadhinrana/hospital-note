<div class="panel-body" style="color: #004876;" id="invoice">
    @include('inc.tabMenu', ['tabMenuPosition' => 1, 'patient_id'=>$prescription->user_id])
        ...
        <tr>
            <td style="width:50%">
                <div style="border: 3px solid #E9F5E7; padding: 1%;">
                    ...
                    <i class="fa fa-pencil pull-right no-print" id="editAllergy" onclick="editAllergyFunction()"></i>
                    @endif
                    <p style="padding:5px">
                        <i class="fa fa-times pull-right no-print" id="cancelAllergy" onclick="cancelAllergy()"
        ...
        <table class="table" style="border-color:#E9F5E7; border-width: 10px">
            ...
            @foreach($drugs as $drug)
                <tr>
                    ...
                    <td  class="no-print"> <a href="#" class="btn btn-warning btn-sm" onclick="editDrugs({{ $drug->id }})"><i class="fa fa-edit"></i> Edit</a> </td>
                </tr>
            @endforeach
            </tbody>
        </table>
    @else
        <table class="table" style="border-color:#E9F5E7; border-width: 10px">
            <tr>
                <th>No Medication Added Yet. <a href="" data-toggle="modal" data-target="#myModal" onclick="clearForm();"> Click to add.</a>
                </th>
            </tr>
        </table>
    @endif
    ...
    <table class="table" style="border-color:#E9F5E7; border-width: 10px">
        ...
        <td>
            @if(auth()->user()->role_id != 4)
            <i class="fa fa-pencil pull-right no-print" id="editSigniture" onclick="editSignitureFunction()"></i>
            @endif
            <p style="padding:5px; display:none" id="signatureSpan">
                <i class="fa fa-times pull-right no-print" id="cancelSigniture" onclick="cancelSignature()"></i>
        ...
        <tr>
            <td colspan="3">
                ...
                <hr>
                @if(auth()->user()->role_id != 4)
                <i class="fa fa-pencil pull-right no-print" id="editComments" onclick="editCommentsFunction(this)"></i>
                @endif
                <p style="padding:5px">
                    <i class="fa fa-times pull-right no-print" id="cancelComments" onclick="cancelComments()"
                       style="display:none"></i>
...
<script>
    ...
    function dragAllergyChange(th) {
        if ($(th).val() != '') {
            ...
            alert('allergy field is required')
        }
    }
    ...
    function clearForm() {
        ...
        $('#drug-form').find('input[name=_method]').remove();
        $('#medication_id').val(null).trigger('change');
        $('#drug-form').attr('action', '{{ route('drugs.store') }}');
    }

    function editDrugs(id) {
        $('#drug-form').prepend('<input type="hidden" name="_method" value="put">');
        $.ajax({
            url: window.location.origin + '/drugs/' + id + '/edit',
            type: 'get',
            success: function (response) {
                if (response.status == 200) {
                    $('#drug-form').attr('action', window.location.origin + '/drugs/' + id);
                    $('#medication_id').val(response.data.medication_id).trigger('change');
                    $('#dosage').val(response.data.dosage);
                    $('#duration_quantity').val(response.data.duration_quantity);
                    $('#dispensed_by').val(response.data.dispensed_by);
                    $('#myModal').modal();
                } else {
                    alert('Error')
                }
