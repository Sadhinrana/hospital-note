    ...
    <div class="panel panel-success">
        <div class="panel-heading">
            <div class="row">
                ...
                <div class="col-md-4">
                    @if(auth()->user()->role_id != 4 && $vital->status != 1)
                    <a href="{{route('vitals.edit', $vital->id)}}" class="btn btn-sm btn-primary btn-block"><i class="fa fa-pencil"></i> Edit vital</a>
                    @endif
                </div>
            </div>
        </div>
    </div>
    @include('inc.tabMenu', ['tabMenuPosition' => 1, 'patient_id' => $vital->user_id])
                ...
                <h2 class="page-header">
                    ...
                    @else
                        @if($vital->company->logo)
                            <img src="{{asset('/storage/'.$vital->company->logo)}}" alt="Company Logo" width="50" height="30">
                        @else
                            <i class="fa fa-building"></i>
                        @endif
                        {{$vital->company->name}}.
