        ...
        <div class="panel-body">
            <form action="{{ route('jobs.update', $job->id) }}" method="post">
                ...
                <div class="form-group row">
                    <div class="col-md-3">
                        <select name="template_id" id="template_id" class="form-control" onchange="showTemplate();checkLengthOfMessage()">
                    ...
                    <div class="col-md-3">
                        <select name="user_id[]" id="user_id" class="form-control" multiple>
                            @foreach($job->doctors as $doctor)
                                @foreach($doctor->user->patients as $patient)
                                    <option value="{{ $patient->id }}" @if($job->users->contains('user_id', $patient->id)) selected @endif>{{ $patient->firstname }} {{ $patient->lastname }}</option>
                                @endforeach
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        <select name="doctor_id" id="doctor_id" class="form-control" onchange="showPatient()">
                    ...
                    <div class="col-md-3">
                        <select name="company_detail_id" id="company_detail_id" class="form-control">
                    ...
                    @else
                        <input type="hidden" name="company_detail_id" value="{{ $companies->first()->id }}">
                    @endif
                </div>
                <div class="form-group row">
                    <div class="col-md-12">
                        <textarea name="message" class="form-control" id="message" rows="5" placeholder="Body" onkeyup="checkLengthOfMessage()">{{ $job->template }}</textarea>
...
@section('js')
    <script>
        ...
        function showPatient() {
            ...
            // Make ajax call to get patient by doctor
            $.ajax({
                type: 'get',
                url: window.location.origin + '/patients-by-doctor/' + $('#doctor_id').val(),
                success: function (response) {
                    if (response.status == 200) {
                        response.data.forEach(function (value, index, array) {
                            $('#user_id').append('<option value="' + value.id + '" selected>' + value.firstname + ' ' + value.lastname + '</option>');
                        });
                    } else {
                        alert('No patient found for this doctor!');
                    }
                }
            });
        }
