...
@section('content')
            ...
            <div class="row">
                ...
                <div class="col-md-3 text-center">
                    @if ($appointment->progress == 'pending')
                        <strong>Current Progress: Pending</strong>
                    @elseif($appointment->progress == 'arrived')
                        <strong>Current Progress: Arrived</strong>
                    @elseif($appointment->progress == 'send_in')
                        <strong>Current Progress: Send In</strong>
                    @elseif($appointment->progress == 'left')
                        <strong>Current Progress: Left</strong>
                    @elseif($appointment->progress == 'did_not_attend')
                        <strong>Current Progress: Did Not Attend</strong>
                    @elseif($appointment->progress == 'cancel')
                        <strong>Current Progress: Cancelled</strong>
                    @else
    ...
    <div class="nav-tabs-custom">
        @include('inc.tabMenu', ['tabMenuPosition' => 4, 'patient_id' => $appointment->user->id])
    </div>
                ...
                <address>
                    <strong>
                        ...
                        <a href="{{route('patients.show',$appointment->user_id)}}">
                            <span style="text-transform: capitalize">{{ $appointment->user->title }}</span> {{$appointment->user->firstname}} {{$appointment->user->lastname}}
                        </a>
                    </strong><br>
        ...
        <div class="row" style="margin: 2%;">
            ...
            <table class="table table-bordered">
                ...
                <tr>
                    ...
                    <td>
                        @if($appointment->is_video_conference)
                            <button type="button" class="btn btn-sm btn-info">Yes</button>
                            @if($appointment->user_id == auth()->id() || $appointment->doctor_id == auth()->id())
                            <a href="{{ $appointment->user_id == auth()->id() ? route('appointment.video.conference', base64_encode($appointment->id)) . '?roomid=' . base64_encode($appointment->id) : route('appointment.video.conference', base64_encode($appointment->id)) }}" type="button" class="btn btn-sm btn-success"><i class="fa fa-video-camera"></i> Join video consultation</a>
                            @endif
                        @else
    ...
    <div class="row no-print" style="padding: 2%;">
        <div class="col-xs-12">
            ...
            @if ($appointment->status == 'open')
                <button type="button" class="btn btn-default" onclick="changeAppointmentProgress('{{ $appointment->id }}', '{{ $appointment->progress }}')">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Change Appointment Status. (
                        ...
                        @elseif($appointment->progress == 'cancel')
                            <strong style="color: white;">Current Progress: Cancelled</strong>
                        @else
