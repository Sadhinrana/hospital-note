        ...
        <div class="panel-body">
            ...
            <table class="table table-hover table-striped table-custom" id="appointments_table" v-if="appointments.length > 0">
                ...
                <tr>
                    ...
                    <th @click="filterParam('sort', 'doctor')" :class="{ 'table-custom-header' : params.sort_by != 'doctor', 'sorting_asc' : params.sort_by == 'doctor' && params.order_by == 'asc', 'sorting_desc' : params.sort_by == 'doctor' && params.order_by == 'desc' }">Staff Assigned</th>
                    <th>Action</th>
                </tr>
                </thead>
                <tbody>
                <tr v-for="appointment in appointments">
                    ...
                    <td v-if="appointment.status == 'open'">
                        ...
                        <a :onclick="'changeAppointmentProgress(' + appointment.id + ',\'' + appointment.progress +'\')'" style="cursor: pointer;"
                           data-toggle="tooltip" title="Click to change appointment progress.">
                            ...
                            <strong v-else-if="appointment.progress == 'cancel'">Cancelled</strong>
                    ...
                    <td>
                        <form v-if="appointment.record" :action="base_url + '/video/appointment/conference/download/' + appointment.record.id" method="POST">
                            {{csrf_field()}}
                            <button type="submit" class="btn btn-sm btn-success"><i class="fa fa-download"></i> Download video</button>
                        </form>
                        <form :action="base_url + '/appointments/' + appointment.id" method="POST">
                            <div v-if="appointment.user_id == {{ auth()->id() }} || appointment.doctor_id == {{ auth()->id() }}" style="margin: 5px 0">
                                <a :href="base_url + '/video/appointment/conference/' + btoa(appointment.id) + '?roomid=' + btoa(appointment.id)" class="btn btn-success btn-sm" v-if="appointment.user_id == {{ auth()->id() }}"><i class="fa fa-video-camera"></i> Join Conference</a>
                                <a :href="base_url + '/video/appointment/conference/' + btoa(appointment.id)" class="btn btn-success btn-sm" v-else><i class="fa fa-video-camera"></i> Join video consultation</a>
                            </div>
