...
@endif
@include('inc.tabMenu', ['tabMenuPosition' => 1, 'patient_id' => $patientTreatment->user_id])
<div class="row" id="newTemplate" style="display: none">
    <div class="col-md-12">
        <div class="box box-success">
            <div class="box-body">
                <form action="{{route('patient_treatment_notes.update', $patientTreatment->id)}}" id="create-template-form" method="post">
                    {{csrf_field()}}
                    @method('put')
                    <input type="hidden" name="user_id" value="{{ $patientTreatment->user_id }}">
                    <div class="row" style="padding: 1%;">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label>Select Template</label>
                                <input type="hidden" name="template_id" value="{{$patientTreatment->template->id}}">
                                <p><b>{{$patientTreatment->template->title}}</b></p>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="appointment_id" id="appointment_id" class="form-control">
                                    ...
                                    @foreach ($patientTreatment->user->appointments as $appointment)
                                        <option value="{{$appointment->id}}" {{ $appointment->id == $patientTreatment->appointment_id ? "selected" : ''}}>{{date('d M Y', strtotime($appointment->appointment_date))}}, {{ $appointment->from }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                                ...
                                <div class="row">
                                    <div v-for="question in section.questions" class="col-md-12">
                                        <div v-if="question.type == 0" class="form-group">
                                            ...
                                            <input type="text" :name="'answer[' + section.id + '][' + question.id + ']'" class="form-control" placeholder="Answer" :value="question.answer ? question.answer.answer : ''" required>
                                        </div>

                                        <div v-if="question.type == 1" class="form-group">
                                            <label for="comment">@{{ question.title }} </label>
                                            <textarea class="form-control" rows="5" :name="'answer[' + section.id + '][' + question.id + ']'" placeholder="Answer" required>@{{ question.answer ? question.answer.answer : '' }}</textarea>
                                        </div>
                                        ...
                                        <div v-if="question.type == 2" class="form-group">
                                            ...
                                            <label v-for="answer in question.answers" class="radio-inline">
                                                <input type="radio" :value="answer.answer" :name="'answer[' + section.id + '][' + question.id + ']'" required :checked="answer.answer == (question.answer ? question.answer.answer : '')">@{{ answer.answer }}
                                            </label>
                                        </div>
                                        ...
                                        <div v-if="question.type == 3" class="form-group">
                                            ...
                                            <label v-for="answer in question.answers" class="checkbox-inline">
                                                <input type="checkbox" :value="answer.answer" :name="'answer[' + section.id + '][' + question.id + '][]'" v-if="question.answer && question.answer.answer.includes(answer.answer)" checked>@{{ answer.answer }}
                                                <input type="checkbox" :value="answer.answer" :name="'answer[' + section.id + '][' + question.id + '][]'" v-else="">@{{ answer.answer }}
                                            </label>
                                        </div>
    ...
    new Vue({
        ...
        methods: {
            getSections() {
                ...
                $.ajax({
                    type: 'get',
                    url: '{{ route('treatmentNote.sections', ['id' => $patientTreatment->id]) }}',
                        ...
                        for(var i = 0; i < _this.sections.length; i++) {
                            ...
                            for(var j = 0; j < questions.length; j++) {
                                if(questions[j].type == 4) {
                                    if (!questions[j]['answer']) break;
                                    var answerString = questions[j]['answer']['answer'];
                                    var answerArray = answerString.split(",");
                                    bodycharts[questions[j]['id']] = [];
                                    for (var k = 0; k < answerArray.length; k++) {
                                        if (answerArray[k] == '') continue;
                                        bodycharts[questions[j]['id']].push(answerArray[k]);
                                    }
                                }
                            }
                        }
                        _this.$set(_this, 'bodycharts', bodycharts);
                    }
                });
            },
            renderSelect(){
            ...
            checkboxAnswerCheck(answer, answer2) {
                if (!answer2) return false;
                var answerString = answer2.answer;
                var answerArray = answerString.split(",");
                var found = false;
