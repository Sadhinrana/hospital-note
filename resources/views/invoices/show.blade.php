...
@section('content')
    ...
    @include('inc.tabMenu', ['tabMenuPosition' => 6, 'patient_id' => $invoice->user_id])
    <section class="invoice" id="invoice">
        ...
        <div class="row invoice-info">
            ...
            <div class="col-sm-5 invoice-col">
                <strong>Client/Patient</strong>
                <address>
                    <strong>
                        @if($invoice->user)
                            {{ ucfirst($invoice->user->title) }}
                            {{$invoice->user->firstname}}
                            {{$invoice->user->lastname}}
                        @else
                            @if($invoice->insurance_name)
                                <br>  Primary ReferReferer Name : {{ $invoice->insurance_name }}
                            @endif
                        @endif
                    </strong><br>
                    @if($invoice->user)
                        ...
                        @if($invoice->insurance_name)
                            <br>  <strong>Invoice To</strong> : {{ $invoice->insurance_name }}
                        @endif
                    @else
        ...
        <div class="row">
            ...
            <div class="col-xs-6">
                ...
                @php
                    $net_total = $invoice->quantity * App\Charge::find($invoice->charge_id)->amount;
                    $tax = $invoice->tax_id ? App\Tax::find($invoice->tax_id)->rate : 0;
                    $tax_amount = ($net_total * $tax) / 100;
                    $currency = App\Currency::find($invoice->currency_id)->symbol;
                @endphp
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tr>
                            <th>Net Amount</th>
                            <td>
                                {{$currency}}{{number_format($net_total,2)}}
                            </td>
                        </tr>
                        @if($invoice->tax_id)
                            <tr>
                                <th>Total VAT {{App\Tax::find($invoice->tax_id)->rate}}%</th>
                                <td>
                        ...
                        <tr>
                            <th>Total</th>
                            <td>
                                {{$currency}}{{ number_format($net_total+$tax_amount,2) }}
                            </td>
                        </tr>
                        </tbody>
                    </table>
