            ...
            if (count(auth()->user()->companies)) {
                foreach (auth()->user()->companies as $company) {
                    if (auth()->user()->role->role_type == 3) {
                        $patients = User::where('role_id', 5)->where('user_id', auth()->id())->count();
                        $appointmentsAll = Appointment::where('doctor_id', auth()->id())->count();
                    } else {
                        $ids = $company->users->pluck('id');
                        $patients = User::where('role_id', 5)->whereIn('user_id', $ids)->count();
                        $appointmentsAll = Appointment::whereIn('doctor_id', $ids)->count();
                    }
                }
            } else {
                $appointmentsAll = Appointment::where('doctor_id', auth()->id())->count();
